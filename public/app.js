console.log("Checkpoint Collective - Master Sürüm (v3.6 - İkonlu) 🚀");
...
async function loadAdminDashboard() {
    ...
    const results = await Promise.all(promises); html += results.join('');
    if (!hasAlerts) html += `<p style="font-size:12px; color:gray; text-align:center;">Bugün için kritik bir durum yok. Takım stabil. 👍</p>`;
...
function loadTemplateToInputs() {
    const selector = document.getElementById('templateSelector'); const selectedId = selector.value;
...
function openWorkoutAssignModal(dateStr) {
    ...
    const chk = document.getElementById('saveAsTemplate'); if (chk) { chk.checked = false; chk.parentElement.style.display = 'flex'; }
...
function saveWorkout() {
    ...
    if (editingWorkoutId) {
        ...
        workoutRef.add({ date: dateStr, title: title, desc: desc, isCompleted: false, stravaLink: "", assignedBy: currentUserId, createdAt: new Date(), reportRpe: 0, reportNote: "" }).then(() => {
            if (saveAsTemplate) { db.collection('templates').add({ title: title, desc: desc, createdAt: new Date() }); }
            closeWorkoutModal(); alert("Antrenman Öğrenciye Gönderildi! 📨");
        });
    }
}
...
function deleteWorkout() {
    if (!activeStudentId || !openWorkoutId) return;
    if (confirm("Bu antrenmanı silmek istediğine emin misin?")) { db.collection('users').doc(activeStudentId).collection('workouts').doc(openWorkoutId).delete().then(() => { closeWorkoutViewModal(); }); }
}
...
function openWorkoutView(workoutId, title, date, desc, isCompleted, stravaLink, ownerId, rpe, note) {
    ...
    const adminActions = document.getElementById('admin-workout-actions'); if (currentUserRole === 'admin') adminActions.style.display = 'flex'; else adminActions.style.display = 'none';
    ...
    if (completed) {
        ...
        if (stravaLink && stravaLink.length > 5) document.getElementById('displayStrava').innerHTML = `<a href="${stravaLink}" target="_blank" style="color:var(--blue); font-size:11px; text-decoration:none;">🔗 Strava'da Aç</a>`;
        else document.getElementById('displayStrava').innerHTML = "";
    } else {
        displayDiv.style.display = 'none'; formDiv.style.display = 'none'; btnOpen.style.display = 'block';
        selectedRpe = 0; document.getElementById('rpeValueDisplay').innerText = "Seçilmedi"; document.querySelectorAll('.rpe-box').forEach(b => b.classList.remove('selected'));
        document.getElementById('reportNote').value = ""; document.getElementById('reportStrava').value = "";
    }
}
...
function submitWorkoutReport() {
    if (!openWorkoutId) return; if (selectedRpe === 0) return alert("Lütfen zorluk derecesini seçin.");
    const note = document.getElementById('reportNote').value; const link = document.getElementById('reportStrava').value; const targetId = activeStudentId || currentUserId;
...
function renderCharts(canvasRpeId, canvasPieId, workouts) {
    const completedWorkouts = workouts.filter(w => w.isCompleted && w.reportRpe > 0).sort((a, b) => a.date.localeCompare(b.date)).slice(-10);
    const rpeLabels = completedWorkouts.map(w => w.date.slice(5)); const rpeData = completedWorkouts.map(w => w.reportRpe);
    ...
    if (ctxRpe) {
        ...
        chartInstances[canvasRpeId] = new Chart(ctxRpe, {
            type: 'line',
            data: { labels: rpeLabels, datasets: [{ label: 'Zorluk', data: rpeData, borderColor: '#FF6B35', backgroundColor: 'rgba(255, 107, 53, 0.1)', borderWidth: 2, tension: 0.3, pointBackgroundColor: '#fff' }] },
            options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, max: 10, grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#888' } }, x: { display: false } } }
        });
    }
...
function loadRaces() {
    db.collection('races').orderBy('date', 'asc').onSnapshot(snapshot => {
        ...
        renderCalendar();
        if (!selectedFullDate) showUpcomingRaces(); else showDayDetails(selectedFullDate);
    });
}
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const race = allRaces.find(r => r.date === fullDate); // İlk yarışı bul
...
function showUpcomingRaces() {
    ...
    document.getElementById('selected-day-races').innerHTML = html; document.getElementById('btnAddRaceToDay').style.display = 'none';
}
    ...
    db.collection('users').doc(targetUserId).collection('my_races').orderBy('date', 'asc').get().then(snap => {
        ...
        snap.forEach(doc => {
            ...
            <div class="my-race-item" style="border-left-color:#4a90e2;">
                <div class="my-race-date"><div class="my-race-day" style="color:#4a90e2;">${d}</div><div class="my-race-month">${m}</div></div>
                <div style="flex:1"><div style="font-weight:bold; font-size:14px;">${r.name}</div><div style="font-size:11px; color:#888;">${r.category}</div></div>
            </div>`;
        });
...
function renderStudentCalendarWithData(workouts) {
    ...
    for (let i = 0; i < startDay; i++) html += `<div class="day-cell empty"></div>`;
    for (let day = 1; day <= daysInMonth; day++) {
        const monthStr = (studentMonth + 1).toString().padStart(2, '0'); const dayStr = day.toString().padStart(2, '0'); const fullDate = `${studentYear}-${monthStr}-${dayStr}`;
        const workout = workouts.find(x => x.date === fullDate);
        let dotClass = ''; if (workout) dotClass = workout.isCompleted ? 'has-workout completed' : 'has-workout';
...
function renderStudentCalendar() { db.collection('users').doc(activeStudentId).collection('workouts').get().then(snap => { const workouts = []; snap.forEach(d => { const dd = d.data(); dd.id = d.id; workouts.push(dd); }); renderStudentCalendarWithData(workouts); }); }
...
function loadMyRaces() {
    ...
    db.collection('users').doc(currentUserId).collection('my_races').orderBy('date', 'asc').onSnapshot(snap => {
        ...
        snap.forEach(d => {
            ...
            // Yarışın tipini allRaces'ten bul (ikon için)
            ...
            <div class="my-race-item">
                <div style="margin-right:10px;"><img src="${iconFile}" class="race-type-icon"></div>
                <div class="my-race-date"><div class="my-race-day">${da}</div><div class="my-race-month">${m}</div></div>
                <div style="flex:1"><div style="font-weight:bold; font-size:14px;">${dat.name}</div><div style="font-size:11px; color:#888;">${dat.category}</div></div>
...
// YENİ KAYIT FONKSİYONU (TİP DAHİL)
function saveRaceFromModal() {
    ...
    const type = document.getElementById('modalRaceType').value; // Seçilen tip
    
    if (!name) return alert("İsim giriniz");
    db.collection('races').add({ name: name, category: cat, date: selectedFullDate, type: type, createdAt: new Date() }).then(closeAddModal);
}
